            ...
            it('retrieves all beads for a specific solution', () => {
                return request.get('/api/beads/1')
                    ...
                    .then(({ body: beads }) => {
                        ...
                        expect(beads[1750].id).to.equal(630);
                        expect(beads[1750].x).to.equal(11);
                        expect(beads[1750].y).to.equal(30);
                        expect(beads[1750].bead_id).to.equal(2);
                        expect(beads[1750].colour_name).to.equal('Cream');
                        expect(beads[1750].r).to.equal(240);
                        expect(beads[1750].g).to.equal(232);
                        expect(beads[1750].b).to.equal(185);
                    });
            });
        });
